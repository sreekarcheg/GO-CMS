...
func courseHandler(writer http.ResponseWriter, request *http.Request) {
	...
	for rows.Next() {
		...
		fmt.Println(titlestring)
		if(role==1 || role == 2){
			rows1, err1 := mysqldb.Query("select count(distinct p.submittorid) from (select submittorid, assignmentid from substring union select submittorid, assignmentid from subfile)p where p.assignmentid=?",assignmentid)
			if err1 != nil {
				fmt.Println(err1)
			...
			if(duetime != ""){
				rows1, err1 := mysqldb.Query("select exists(select p.* from (select submittorid, assignmentid from substring union select submittorid, assignmentid from subfile)p where p.submittorid=? and p.assignmentid=?)", accountID, assignmentid)
				if err1 != nil {
					fmt.Println(err1)
					fatalQueryError("Assignment completion status in course page")
				}
